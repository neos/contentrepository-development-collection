  ...
  *without recursion*
  ...
  Background:
    ...
    And the event NodeAggregateWithNodeWasCreated was published with payload:
      ...
      | originDimensionSpacePoint     | {"language":"mul"}                                       |
      | coveredDimensionSpacePoints   | [{"language":"mul"},{"language":"en"},{"language":"de"}] |
      | parentNodeAggregateIdentifier | "nodimer-tetherton"                                      |
  ...
  Scenario: Increase node aggregate coverage
    When the command IncreaseNodeAggregateCoverage is executed with payload:
      ...
      | initiatingUserIdentifier  | "initiating-user-identifier" |
      | recursive                 | false                        |
    # 1 for the root workspace, 5 for the initial nodes, 1 for sir-david-nodenborough
    ...
    And I expect node aggregate identifier "nodimer-tetherton" and path "document/tethered/tethered-leaf" to lead to node {"contentStreamIdentifier":"cs-identifier", "nodeAggregateIdentifier":"nodimer-tetherton", "originDimensionSpacePoint": {"language":"mul"}}
    And I expect node aggregate identifier "nody-mc-nodeface" and path "document/tethered/tethered-leaf/child-document" to lead to node {"contentStreamIdentifier":"cs-identifier", "nodeAggregateIdentifier":"nody-mc-nodeface", "originDimensionSpacePoint": {"language":"mul"}}
    ...
    And I expect node aggregate identifier "nodewyn-tetherton" and path "document/tethered" to lead to node {"contentStreamIdentifier":"cs-identifier", "nodeAggregateIdentifier":"nodewyn-tetherton", "originDimensionSpacePoint": {"language":"mul"}}
    And I expect node aggregate identifier "nodimer-tetherton" and path "document/tethered/tethered-leaf" to lead to node {"contentStreamIdentifier":"cs-identifier", "nodeAggregateIdentifier":"nodimer-tetherton", "originDimensionSpacePoint": {"language":"mul"}}
    And I expect node aggregate identifier "nody-mc-nodeface" and path "document/tethered/tethered-leaf/child-document" to lead to no node
