 ...
 */
final class CollectionTypeDenormalizer implements DenormalizerInterface, SerializerAwareInterface, CacheableSupportsMethodInterface
{
    /**
    ...
    public function denormalize($data, string $type, string $format = null, array $context = []): array
    {
        ...
        if (!TypeHandling::isCollectionType($actualDataType)) {
            throw new InvalidArgumentException(sprintf('Data expected to be a collection type, given: %s', get_debug_type($data)), 1596463272);
        }
        try {
            $parsedType = TypeHandling::parseType($type);
        } catch (InvalidTypeException $e) {
            throw new InvalidArgumentException(sprintf('Failed to parse data type "%s": %s', $type, $e->getMessage()), 1596466324);
        }
        if (empty($parsedType['elementType'])) {
            throw new InvalidArgumentException(sprintf('Type expected to be a generic collection type in the format array<elementType>, given: %s', $type), 1596466387);
        }
    ...
    {
        if ($this->serializer === null) {
            throw new BadMethodCallException(sprintf('The serializer needs to be set to allow "%s()" to be used.', __METHOD__), 1596465675);
        }
        try {
            $parsedType = TypeHandling::parseType($type);
        } catch (InvalidTypeException $e) {
    ...
    public function setSerializer(SerializerInterface $serializer): void
    {
        if (!$serializer instanceof DenormalizerInterface) {
            throw new InvalidArgumentException(sprintf('Expected a serializer that also implements the %s.', DenormalizerInterface::class), 1596464789);
        }
